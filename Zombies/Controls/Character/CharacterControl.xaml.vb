...
Public Class Character
    ...
    'Angle and positioning for updating the character visually
    Public Property Angle As Double = 0
...
Public Class CharacterControl
    Public Data As New Character()
    'BEFORE DEBUG CHANGE TO PROPERTY AND CHANGE SYNC VALUES
...
#Region "Properties"
    ...
    Public Event SyncChanges()
    Public GunSounds As New System.Media.SoundPlayer(My.Computer.FileSystem.CurrentDirectory & "\Sounds\" & Data.Gun & ".wav")
    ...
    Sub LoadNewGun(gunString As String, ByVal bullets As Integer, clipamount As Integer, strength As Integer)
        ...
        Data.BulletsInClip = clipamount
        Data.BulletStrength = strength
        RaiseEvent OnLoad()
        RaiseEvent OnReload()
    End Sub
    Public Property BulletAmount As Integer
        ...
        Set(value As Integer)
            If Data.Bullets <> value Then 'If the bullets have changed
                If Data.Bullets = 0 And value > 0 Then 'Detect if we get new ammo after we had no ammo
                    RaiseEvent OnNewAmmo()
                ElseIf Data.Bullets <> (value + 1) Then
                    'Assume we've changed bullet amounts on new gun
                    RaiseEvent OnReload()
                End If
                Data.Bullets = value 'Then we'll change the amount
                RaiseEvent OnBulletChange() 'But we'll also have to change the bullet container to show the amount
                Dim CheckBullets As Double = Data.Bullets / Data.BulletsInClip 'Get the amount of bullets in decimal
                If CheckBullets = Int(CheckBullets) Then 'So that we can check if we need to reload (in other words, when the number = .0 then we know that we need to change clips)
                    If value > 0 Then 'Only reload if the number of bullets is higher than 0
                        Data.Reloading = True 'prevents bullets being used when we're reloading
    ...
    Public Property Kills As Integer
        ...
        End Get
        Set(value As Integer)
            If value <> Data.Kills Then
                Data.Kills = value
                RaiseEvent SyncChanges()
            End If
        End Set
    End Property
    Public Property Talking As Boolean
        Get
            Return Data.Talking
        End Get
        Set(value As Boolean)
            If Data.Talking <> value Then
                Data.Talking = value
                RaiseEvent SyncChanges()
            End If
        End Set
    ...
    Public Property Points As Integer
        ...
        End Get
        Set(value As Integer)
            Dim PointsOld As Integer = Data.Points
            Data.Points = If(value < 0, 0, value)
            RaiseEvent OnPointChange(Data.Points, value - PointsOld)
            RaiseEvent SyncChanges()
        End Set
    End Property
    ...
    Sub ShootBullet(Character As CharacterControl)
        If Character.BulletAmount > 0 And Not Character.Data.Reloading Then
            ...
            AddHandler ShootBullet.ShotZombie, Sub(Target As Zombie)
                                                   ...
                                                   Target.Health -= Character.Data.BulletStrength
                                               End Sub
            Character.GunSounds.Play()
            If Character.Data.Gun <> "MP5" Then
                Shoot.Stop()
                Shoot.Interval = 1
            End If
            Shoot.Interval = 120
